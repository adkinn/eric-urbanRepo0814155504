...
The example desktop application sends authentication requests to the Microsoft account and [!INCLUDE[brand](../guides/includes/brand.md)] services for the user credentials that you provide, and then gets the accounts that the authenticated user can access. For more information, please see [Setting Up the Development Environment](../guides/getting-started-using-java-with-bing-ads-services.md#requirements). You can create the example step by step as described below, or start with the [provided examples](http://go.microsoft.com/fwlink/?LinkId=525443).

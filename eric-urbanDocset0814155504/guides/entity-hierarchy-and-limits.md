...
The following figure shows the high level [!INCLUDE[brand](../guides/includes/brand.md)] entity object model from the perspective of one customer that has one or more accounts. For information on services available to manage [!INCLUDE[brand](../guides/includes/brand.md)] entities, see [Bing Ads API Overview](../guides/bing-ads-api-overview.md).
...
For more information about managing negative keywords and negative keyword lists, please see the technical guide about [Negative Keywords](../guides/negative-keywords.md).
...
Product groups are used to specify which products from your Bing Merchant Center catalog should be included in a particular ad group. In order to understand product groups, first make sure you are familiar with [Product Ads](../guides/product-ads.md).

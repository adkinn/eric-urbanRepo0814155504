...
ms.tgt_pltfrm: ""
ms.topic: "article"
ms.assetid: fde0dd27-baa3-454a-888b-b3b7fecd749e
caps.latest.revision: 35
author: "eric-urban"
ms.author: "scottwhi"
---
# Managing your Products
Content API is a RESTful API that uses the [Products](../content-api/products-resource.md) resource to manage product offerings in your Bing Merchant Center (BMC) store. 
...
Each HTTP request must include the user's credentials and your developer token. To specify the user's credentials, set either the [AuthenticationToken](../content-api/products-resource.md#authtoken) header or the [UserName](../content-api/products-resource.md#username) and [Password](../content-api/products-resource.md#password) headers, but not both. 

To specify your developer token, set the [DeveloperToken](../content-api/products-resource.md#devtoken) header.

If you manage catalogs on behalf of other customers, you must set the [CustomerId](../content-api/products-resource.md#customerid) header to the customer ID of the customer whose store you're managing, and the [CustomerAccountId](../content-api/products-resource.md#customeraccountid) header to the account ID of any of the customer's accounts that you manage (it doesn't matter which managed account).

By default, the Content API uses JSON objects to represent the product offer. To use XML, set the [alt](../content-api/products-resource.md#alt) query parameter to XML.
...
Set `{bmcMerchantId}` to your BMC store ID and set `{productUniqueId}` to the fully qualified product [ID](../content-api/products-resource.md#productid) (for example, Online:en:US:Sku123), not the product's [offerId](../content-api/products-resource.md#offerid). Because the product ID is case sensitive, use the ID that the API returned to you when you added the product. 

Send an HTTP GET request to the resulting URL. If the product was found, the response will contain a [Product](../content-api/products-resource.md#product) object that contains the offer as last inserted or updated.
...
For a code example that shows how to get a product offer, see [Getting a Product Offer](../content-api/code-examples.md#get).
...
To page through the list of offers, use the [max-results](../content-api/products-resource.md#maxresults) and [start-token](../content-api/products-resource.md#starttoken) query parameters. In your first call, set `max-results` to the maximum number of offers that you want the service to return. The maximum number of offers that the service can return is 250 and the default is 25. The following shows an example of the initial call.

`https://content.api.bingads.microsoft.com/shopping/v9.1/bmc/{bmcMerchantId}/products?max-results=250` 

Send an HTTP GET request to the resulting URL. If the store contains offers, the response will contain a [Product](../content-api/products-resource.md#product) object that contains up to the requested number of offers.

If there are more offers available, the response will include the `nextPageToken` field (see [Products](../content-api/products-resource.md#products)), which contains the token value that you use to set the `start-token` parameter to in your next List request. The following shows an example that uses the token.

`https://content.api.bingads.microsoft.com/shopping/v9.1/bmc/{bmcMerchantId}/products?max-results=250&start-token=DFSos893j...`

For a code example that shows how to get a list of product offers, see [Getting a List of Product Offers](../content-api/code-examples.md#list).
...
For a code example that shows how to delete a product offer, see [Deleting a Product Offer](../content-api/code-examples.md#delete).
...
If you send an HTTP POST request to the resulting URL, the offer will be written to the default store catalog. To write the offer to a specific catalog, include the [bmc-catalog-id](../content-api/products-resource.md#bmccatalogid) query parameter as shown in the following example.

`https://content.api.bingads.microsoft.com/shopping/v9.1/bmc/{bmcMerchantId}/products?bmc-catalog-id=123456` 

If the product was inserted, the response will contain a [Product](../content-api/products-resource.md#product) object. The `Product` object will include the product [ID](../content-api/products-resource.md#productid), which you use to get and delete the offer.

An offer must include the following fields:

* [availability](../content-api/products-resource.md#availability)
* [channel](../content-api/products-resource.md#channel)
* [condition](../content-api/products-resource.md#condition)
* [contentLanguage](../content-api/products-resource.md#contentlanguage)
* [imageLink](../content-api/products-resource.md#imagelink)
* [link](../content-api/products-resource.md#link)
* [offerId](../content-api/products-resource.md#offerid)
* [price](../content-api/products-resource.md#price)
* [targetCountry](../content-api/products-resource.md#targetcountry)
* [title](../content-api/products-resource.md#title)
...
>[!CAUTION] Be sure to use the same casing for `channel`, `contentLanguage`, `targetCountry`, and `offerId` because you can effectively add the same product multiple times if the casing is different.
...
You must specify the values if known. If you do not specify any of them, you must set the [identifierExists](http://msdn.microsoft.com/en-us/e431640b-fa1a-4161-8272-8b962b72ee7a) field to **false**. The default is **true**.
...
For apparel products that are available in multiple colors, patterns or materials, create a product for each variant and use the [itemGroupId](../content-api/products-resource.md#itemgroupid) field to group all product variants.

For a code example that shows how to insert a product offer, see [Inserting a Product Offer](../content-api/code-examples.md#insert).
...
If you send an HTTP POST request to the resulting URL, the insert operations will be applied to the default store catalog. To apply the offers to a specific catalog, include the [bmc-catalog-id](../content-api/products-resource.md#bmccatalogid) query parameter as shown in the following example.

`https://content.api.bingads.microsoft.com/shopping/v9.1/bmc/{bmcMerchantId}/products/batch?bmc-catalog-id=123456`

The body of the POST is a [Batch](../content-api/products-resource.md#batch) object which contains an array of [Item](../content-api/products-resource.md#item) objects. For all operations, set the `batchId`, `merchantId` and `method` fields. The `batchId` is a user-defined ID that you use to identify the batch item in the response; the `merchantId` is your store ID; and the `method` is the operation to be performed (for example, insert, delete or get).

If `method` is insert, set the `product` field to a [Product](../content-api/products-resource.md#product) object which contains the offer to add or update. Otherwise, if `method` is a get or delete, set `productId` to the fully qualified [ID](../content-api/products-resource.md#productid) of the offer that you want to get or delete.
...
For a code example that shows how to use batch processing, see [Creating a Batch Request](../content-api/code-examples.md#batch).

...
These costs occur whether you process a single item or a set of items. To minimize overhead, in general, you should try to process as many items in one call as possible. For example, instead of calling [UpdateCampaigns](~/campaign-api/updatecampaigns-service-operation.md) for each campaign that you want to update, call it only once for multiple campaigns that you want to update. To manage large scale data you should use the [Bulk service](~/bulk-api/bulk-service-reference.md). The Bulk service allows you to download data as a TSV or CSV file, modify it as needed, and then upload your changes. For more information about using the Bulk service, see [Bulk Download and Upload](../concepts/bulk-download-and-upload.md).
...
### Bulk
The Bulk service limits the number of requests that you can make to [DownloadCampaignsByAccountIds](~/bulk-api/downloadcampaignsbyaccountids-service-operation.md), [DownloadCampaignsByCampaignIds](~/bulk-api/downloadcampaignsbycampaignids-service-operation.md), and [GetBulkUploadUrl](~/bulk-api/getbulkuploadurl-service-operation.md). The details of the service limits are internal and subject to change.

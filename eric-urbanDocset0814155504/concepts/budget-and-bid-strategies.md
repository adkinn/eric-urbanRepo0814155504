...
> [!IMPORTANT]
> To avoid a breaking change when updating campaigns, you might need to code for shared budgets in the Bing Ads platform, even if you do not plan to use shared budgets. For more details and to determine whether the campaign uses a shared budget, check the value of the *BudgetId* element ([Campaign](~/campaign-api/campaign-data-object.md) object) or *Budget Id* field (Bulk ([Campaign](~/bulk-api/campaign.md) object) record).

The Bing Ads API supports the *DailyBudgetAccelerated* and *DailyBudgetStandard* values as defined in the [BudgetLimitType](~/campaign-api/budgetlimittype-value-set.md) value set.
...
> [!NOTE]
> The Bing Ads web application uses the term *Bid strategy*, the Bing Ads Bulk API uses the *Bid Strategy Type* column for upload and download, and the Bing Ads Campaign Management API derives several bid strategy objects from the [BiddingScheme](~/campaign-api/biddingscheme-data-object.md) object.
...
> 
> Also note that you must have conversion tracking (a UET tag and a conversion goal) set up for the *EnhancedCpc*, *MaxConversions*, and *TargetCpa* bid strategy types to work. See [Universal Event Tracking](../concepts/universal-event-tracking.md) for more information.
> 
...
-   For keywords that you don't want leading to clicks, add them to your keyword list as negative keywords. For more information, see [Negative Keywords](../concepts/negative-keywords.md).  
